...
<svg aria-hidden="true" style="position:absolute;width:0;height:0;overflow:hidden;" xmlns="http://www.w3.org/2000/svg">
  <defs>
    ...
    >
      <?php if (in_array($order['status'], ['new', 'pending'])): ?>
      <div class="invoice__progress">
          <div
          class="invoice__progressBar"
            ...
            aria-valuemax="100"
            style="width: 0"
            data-expire-utc="<?php echo (float)$order['expire_utc']; ?>"
          >
      ...
      <div class="invoice__content">
        <?php if ($order['status'] == 'new' || ($order['status'] == 'pending' && $order['pending_amount'] > 0)): ?>
        <div class="invoice__rowInfo">
          <div class="invoice__infoOrder">
            <strong><small>Order #</small><?php echo htmlspecialchars($order['order_id']); ?></strong>
            </div>
          <div class="invoice__infoAmount">
                <div class="invoice__amountSum">
                  <strong><?php echo number_format((float)$order['amount'], 8, '.', ''); ?><?php echo htmlspecialchars($order['currency']); ?></strong> <br>
                    <?php echo number_format((float)$order['amount'] / (float)$order['source_rate'], 2, '.', ''); ?> <?php echo htmlspecialchars($order['source_currency']); ?>
                </div>
              </div>
            </div>
            <img
          ...
          alt="Invoice QR code"
              ...
              height="160"
            >
        <p class="invoice__hint">Send the indicated amount to the address below:</p>


        <div class="invoice__formGroup">
          <div class="invoice__inputGroup">
          <div class="invoice__inputGroupAddon">
              <img
              ...
              <input
                type="text"
              class="invoice__formControl"
                value="<?php echo htmlspecialchars($order['wallet_hash']); ?>"
                readonly
              onclick="copyInvoiceValue(this)"
              >
            <span
              class="invoice__btn"
              onclick="copyInvoiceValue(this)"
        ...
        <div class="invoice__formGroup">
          <div class="invoice__inputGroup">
            <div class="invoice__inputGroupAddon"><?php echo htmlspecialchars($order['currency']); ?></div>
              <input
                type="text"
              class="invoice__formControl invoice__pendingAmount"
                value="<?php echo number_format((float)$order['pending_amount'], 8, '.', ''); ?>"
                readonly
              onclick="copyInvoiceValue(this)"
              >
            <span
        ...
        <?php elseif ($order['status'] == 'pending' && $order['pending_amount'] <= 0): ?>
          <div class="invoice__result invoice__row_center_vertical">
            <svg class="invoice__icon_status _loader" xmlns="http://www.w3.org/2000/svg">
              ...
              <?php $order['stringConfirm'] = (float)$order['expected_confirmations'] > 1 ? 'confirmations' : 'confirmation' ?>
            <h3>Waiting for <?php echo (float)$order['expected_confirmations'] - (float)$order['confirmations']; ?>
              of <?php echo (float)$order['expected_confirmations']; ?> <?php echo $order['stringConfirm']; ?></h3>
